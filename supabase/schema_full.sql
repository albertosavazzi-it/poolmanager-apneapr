-- ==========================================
-- SCRIPT SCHEMA COMPLETO PER NUOVO DB SUPABASE
...
-- 4. Create entrance_credits table (tracks credits added by admin with payment info)
...
-- 5. Create entrance_logs table (tracks when users use entrances)
...
CREATE POLICY "Admins can delete entrance logs"
  ON public.entrance_logs FOR DELETE
  TO authenticated
  USING (has_role(auth.uid(), 'admin'::app_role));
